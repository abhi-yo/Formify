(function () {
  ...
  class FormifySDK {
    constructor(apiKey, options = {}) {
      ...
      this.log("Formify SDK initialized", {
    ...
    async submitForm(form, options = {}) {
      try {
        ...
        let payload = {
          ...
          timestamp,
          signature: await this.generateHMAC(
            dataToSign,
            options.secretKey || ""
          ),
        };
        ...
        if (
          response.status === 400 &&
          result.error === "Proof of work required"
        ) {
          this.log("Proof of work required, solving challenge...");
          const challenge = result.challenge;
          this.proofOfWork = await this.solveProofOfWork(
            challenge.challenge || Math.random().toString(),
